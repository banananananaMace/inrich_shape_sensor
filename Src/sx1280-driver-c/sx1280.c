    ...
    if( packetType==PACKET_TYPE_LORA )
    {
        ...
        case MODE_RX: //接收模式下
            if( ( irqRegs & IRQ_RX_DONE ) == IRQ_RX_DONE )//接收完成
            {
                ...
                {
                    AppState = APP_RX_ERROR;//调用接收错误完成函OnRxError，即：AppState = APP_RX_ERROR;
                }
                else  //接收成功
                {
                    AppState = APP_RX; //调用接收完成函数OnRxDone，即：AppState = APP_RX;
                }
            }
            ...
            {
                AppState = APP_RX_ERROR;  //调用接收错误函数：OnRxError 即AppState改为APP_RX_ERROR，同时printf 发送错误提示
            }
            if( ( irqRegs & IRQ_RX_TX_TIMEOUT ) == IRQ_RX_TX_TIMEOUT ) //接收发送超时
            {
                AppState = APP_RX_TIMEOUT;// /调用接收超时函数OnRxTimeout AppState = APP_RX_TIMEOUT;
            }
            if( ( irqRegs & IRQ_RANGING_SLAVE_REQUEST_DISCARDED ) == IRQ_RANGING_SLAVE_REQUEST_DISCARDED ) //
            {
                AppState = APP_RX_ERROR;//调用接收错误函数：OnRxError 即AppState改为APP_RX_ERROR，同时printf 发送错误提示
